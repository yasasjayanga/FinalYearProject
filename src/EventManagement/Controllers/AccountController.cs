...
namespace EventManagement.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, IFormFile file, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    //var current_User = await _userManager.FindByNameAsync(model.UserName);
                    ...
                    if (file != null)
                    {
                        fileName = currentUser.Id + "~" + file.FileName;
        ...
        public async Task<IActionResult> ManageProfile()
        {
            ...
            var cuser = new UsersService().getUser(user.Id);
            Mapper.Map(cuser, model);
        ...
        [HttpPost]
        public async Task<IActionResult> ManageProfile(RegisterViewModel model, IFormFile file)
        {
            string fileName = string.Empty;
            if (file != null)
            {
                var path = Path.Combine(_environment.WebRootPath, "uploads");
                fileName = model.UserId + "~" + file.FileName;
                using (var fileStream = new FileStream(Path.Combine(path, fileName), FileMode.Create))
                {
                    await file.CopyToAsync(fileStream);
                }
            ...
            var cuser = new User();
            Mapper.Map(model, cuser);
            cuser.ProfilePhoto = fileName;
            cuser.Dob = DateTime.Now;
            new UsersService().UpdateUser(cuser);
            return View(model);
        }
    }
}
